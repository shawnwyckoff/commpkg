...
func (lgz *Logz) Logging(log logger.Log) error {
	return nil
}

func (lgz *Logz) LoggingEx(level logger.Level, text string, tags map[string]string) error {
	return nil
}
...
func (lgz *Logz) getFile(tm time.Time) (*os.File, error) {
	if len(lgz.conf.SaveDir) == 0 {
		return nil, errorz.New("Empty log dierctory")
	}
...
func (lgz *Logz) Init(config *Config) error {
	...
	if lgz.conf.SaveDisk {
		...
		if !pi.Exist {
			return errorz.New(lgz.conf.SaveDir + " create failed")
		}
		if !pi.IsFolder {
			return errorz.New(lgz.conf.SaveDir + " create failed because it is not a folder")
		}
	}
	return nil
}
...
func (lgz *Logz) WriteMsg(when time.Time, msg string, level logger.Level) error {
...
func (lgz *Logz) logging(message string, level logger.Level) {
...
func (lgz *Logz) Debgf(format string, a ...interface{}) {
	lgz.logging(fmt.Sprintf(format, a...), logger.LevelDebg)
}

func (lgz *Logz) Infof(format string, a ...interface{}) {
	lgz.logging(fmt.Sprintf(format, a...), logger.LevelInfo)
}

func (lgz *Logz) Warnf(format string, a ...interface{}) {
	lgz.logging(fmt.Sprintf(format, a...), logger.LevelWarn)
}

func (lgz *Logz) Errof(format string, a ...interface{}) {
	lgz.logging(fmt.Sprintf(format, a...), logger.LevelErro)
}

func (lgz *Logz) Fataf(format string, a ...interface{}) {
	lgz.logging(fmt.Sprintf(format, a...), logger.LevelFata)
}

func (lgz *Logz) Erro(err error, wrapMsg ...string) {
	stack := errorz.GetStack(err)
	if len(wrapMsg) > 0 {
		lgz.logging(strings.Join(append([]string{}, wrapMsg...), ",")+": "+err.Error()+"\nstack: "+stack, logger.LevelErro)
	} else {
...
func (lgz *Logz) Fata(err error, wrapMsg ...string) {
	if len(wrapMsg) > 0 {
		lgz.logging(strings.Join(append([]string{}, wrapMsg...), ",")+": "+err.Error(), logger.LevelFata)
	} else {
		lgz.logging(err.Error(), logger.LevelFata)
	}
}
