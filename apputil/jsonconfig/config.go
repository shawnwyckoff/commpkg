...
func DefaultConfig() (string, error) {
	...
	if (runtime.GOOS == "windows" && stringz.EndWith(fnLower, ".exe")) ||
		(runtime.GOOS == "darwin" && stringz.EndWith(fnLower, ".app")) ||
		(runtime.GOOS == "linux" && stringz.EndWith(fnLower, ".bin")) {
		fn = fn[0 : len(fn)-4]
	}
...
func Unmarshal(filename string, v interface{}) error {
	typeName, isPtr := interfaces.Parse(v)
	if !isPtr {
