...
func FromBytes(size float64) (Speed, error) {
	if size < 0 {
		return Speed(0), errors.New("Negative speed error")
	}
...
func (s *Speed) UnmarshalJSON(b []byte) error {
	str := string(b)
	if len(str) <= 1 {
		return errors.Errorf("invalid json speed '%s'", s)
	}
...
func ParseString(speed string) (Speed, error) {
	...
	if len(s) == 0 {
		return Speed(0), errors.New("speed string \"" + speed + "\" is empty")
	}
	...
	if nonDigitPos < 1 {
		return Speed(0), errors.New("speed string \"" + speed + "\" lack of speed unit part")
	}
	...
	if stringz.EndWith(strings.ToLower(unitStr), "/s") || stringz.EndWith(strings.ToLower(unitStr), "ps") {
		unitStr = stringz.RemoveTail(unitStr, 2)
	}
	if len(unitStr) == 0 {
		return Speed(0), errors.New("speed string \"" + speed + "\" has no unit")
	}
	// Parse "k" "m" "g" "t"...
	...
	if len(unitStr) == 1 { // such like "2M", unit string is "M", its length is 1
		...
		if unitStr == "B" || strings.ToLower(unitStr) == "byte" || strings.ToLower(unitStr) == "bytes" {
			...
			return Speed(0), errors.New("speed string \"" + speed + "\" unit syntax error")
		}
	}
	...
	default:
		return Speed(0), errors.New("speed string \"" + speed + "\" is invalid speed string")
	}
}
