...
func NewScoreCalc(zeroScore, fullScore float64) (*ScoreCalc, error) {
	if zeroScore >= fullScore {
		return nil, errors.Errorf("Invalid valueOfZeroScore(%f) and valueOfFullScore(%f)", zeroScore, fullScore)
	}
...
func (s *ScoreCalc) GetScore() (float64, error) {
	...
	if totalScale != 100 {
		return 0, errors.Errorf("Correct total scale 100, but get %d", totalScale)
	}
