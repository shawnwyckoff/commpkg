...
func SubstrAscii(s string, begin, end int) (string, error) {
	if begin < 0 || begin > (len(s)-1) {
		return "", errors.New("begin index is error")
	}
	if end < 0 || end > (len(s)-1) {
		return "", errors.New("end index is error")
	}
	if begin > end {
		return "", errors.New("begin is bigger than end")
	}
	rs := []rune(s)
...
func SubstrBetween(s string, begin string, end string,
	...
	if len(s) == 0 || len(begin) == 0 || len(end) == 0 {
		return "", errors.New("SubstrAscii has invalid input")
	}
	...
	if a < 0 {
		return "", errors.New("can't find begin string")
	}
	...
	if b < 0 {
		return "", errors.New("can't find end string")
	}
	if !includBegin {
		a += len(begin)
	}
	...
	if a > b {
		return "", errors.New("begin index is bigger than end")
	}
...
func SubstrBetweenUTF8(s string, begin string, end string,
	...
	if LenUTF8(s) == 0 || LenUTF8(begin) == 0 || LenUTF8(end) == 0 {
		return "", errors.New("SubstrByteUTF8 has invalid input")
	}
	...
	if a < 0 {
		return "", errors.New("cann't find begin string")
	}
	...
	if b < 0 {
		return "", errors.New("cann't find end string")
	}
	if !includBegin {
		a += LenUTF8(begin)
	}
	...
	if a > b {
		return "", errors.Errorf("begin index [%d] is bigger than end [%d]", a, b)
	}
...
func ReplaceWithTags(s string, begin string, endAfterBegin string, replaceStr string, replaceTime int) (string, error) {
	if len(s) == 0 || len(begin) == 0 || len(endAfterBegin) == 0 {
		return "", errors.New("ReplaceWithTags has invalid input")
	}
	if replaceTime <= 0 {
