...
func (q *TaskQueue) GetStatistic() *Statistic {
	...
	res.LatelyAvgExecDuration = clock.NsecToDuration(q.tempLatelyExecDurationSum.Nanoseconds() / q.tempLatelyExecCount)
...
func (q *TaskQueue) Pop2do() (*Task, error) {
	...
	return nil, errors.Errorf("No to2Do item")
}
...
func (q *TaskQueue) PopDone() (*Task, error) {
	if len(q.done) == 0 {
		return nil, errors.Errorf("No done item")
	}
	return <-q.done, nil
}
