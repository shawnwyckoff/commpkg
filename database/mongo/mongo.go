...
import (
	...
	"github.com/shawnwyckoff/commpkg/dsa/ranges"
...
func (c *Conn) DBExist(dbName string) (bool, error) {
	dbNames, err := c.DBNames()
	if err != nil {
		return false, err
	}
	return stringz.CountByValue(dbNames, dbName) > 0, nil
}
...
func (d *Database) Exist() (bool, error) {
	return d.c.DBExist(d.inDb.Name())
}
...
func (d *Database) CExists(collName string) (bool, error) {
	if colls, err := d.CNames(); err != nil {
		return false, err
	} else {
		return stringz.CountByValue(colls, collName) == 1, nil
	}
}
...
func (c *Coll) Exist() (bool, error) {
	return c.db.CExists(c.inColl.Name())
}
...
// NOTE: if timePath/Collection/Database doesn't exist or empty collection, returns [nil, ErrNotFound]
...
func (c *Coll) minMax(itemPath string, typeSample interface{}, max bool) (interface{}, error) {
	...
	if cur, err := c.inColl.Find(context.Background(), bson.D{} /*can't input nil, otherwise will return error "document is nil"*/, &opts); err != nil {
		...
		if cur.Next(context.Background()) {
			...
			errTypeNotMatch := errors.Errorf("type(%s) wanted but in database it is not", interfaces.Type(typeSample))
			errTypeNotSupported := errors.Errorf("type(%s) not supported in min/max query", interfaces.Type(typeSample))

			switch interfaces.Type(typeSample) {
			case "time.Time":
			...
			return nil, errorz.ErrNotFound // this is not a really error
		}
	}
}
...
// count by meta data
// if DB / Coll doesn't exist, returns valid 0 and nil error
//
// WARNING:
...
func (mg *Conn) CopyColl(fromDb, fromColl, toDb, toColl string) (cloneCount int, err error) {
	from := mg.DB(fromDb).C(fromColl)
	to := mg.DB(toDb).C(toColl)
...
func (c *Coll) VerifyContinuousInt64Id() (*ranges.RangeFilter, error) {
	...
	correctRf := ranges.NewRangeFilter()
	correctRf.AddRange(ranges.NewRange(minId, maxId))

	infactRf := ranges.NewRangeFilter()
	cur, err := c.Find(nil)
	if err != nil {
		return nil, err
	}
	...
	for cur.Next() {
		...
		if id.Id == -1 {
			return nil, errors.Errorf("decoded -1 id")
		}
