...
import (
	...
	"github.com/shawnwyckoff/gopkg/container/grange"
...
func (c *Coll) minMax(itemPath string, typeSample interface{}, max bool) (interface{}, error) {
	...
	if cur, err := c.inColl.Find(context.Background(), bson.D{} /*can't input nil, otherwise will return error "document is nil"*/, &opts); err != nil {
		...
		if cur.Next(context.Background()) {
			...
			errTypeNotMatch := errors.Errorf("type(%s) wanted but in database it is not", ginterface.Type(typeSample))
			errTypeNotSupported := errors.Errorf("type(%s) not supported in min/max query", ginterface.Type(typeSample))
			...
			return nil, gerror.ErrNotFound // this is not a really error
		}
	}
}
