...
func IsExecutableUnstable2(filename string) (bool, error) {
	...
	if sz > errIfBiggerThan {
		return false, errors.Errorf("%s it bigger than %s", filename, errIfBiggerThan.String())
	}*/
	b, err := fs.FileToBytes(filename)
	if err != nil {
		return false, err
	}
...
func IsExecutable(filename string) (bool, error) {
	...
	if err != nil {
		if vol, err2 := fs.GetFileSize(filename); err2 == nil && vol == volume.Volume(0) {
			return false, nil
		}
		return false, err
	}
...
func DetectFileFormat(filename string) (*FileFormat, error) {
	rd, fd, err := fs.FilenameToReader(filename)
	if err != nil {
		return nil, err
	}
	defer fd.Close()
	...
	if len(r) == 0 {
		return nil, errors.Errorf("detect failed")
	}
	return &r[0], nil
}
