...
func GetSharedUserDir() (string, error) {
	...
	return "", errors.Errorf("GetSharedUserDir() doesn't support %s", runtime.GOOS)
}
...
func GetGoRoot() (string, error) {
	...
	gorootbuf, err := cmd.ExecWaitReturn("go", "env", "GOROOT") // 目前返回的结果最后会多一个换行符，是ExecWait的bug还是本来应该如此，尚不明确
	if err != nil {
		return "", err
	}
...
func GetGoPath() (string, error) {
	gopathbuf, err := cmd.ExecWaitReturn("go", "env", "GOPATH")
	if err != nil {
		return "", err
	}
