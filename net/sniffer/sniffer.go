...
import (
	...
	"github.com/shawnwyckoff/commpkg/net/socks5"
...
func tryGetHTTP2Request(r io.Reader) (fields [][2]string, err error) {
	if !hasHTTP2Preface(r) {
		return nil, errors.Errorf("not HTTP 2")
	}
...
func tryGetSocks5Request(r io.Reader) (socks5target string, err error) {
	errNotSocks5 := errors.Errorf("not socks5 protocol")
	buf := make([]byte, 4)
	...
	if socks5.Command(buf[1]) < socks5.CommandMin || socks5.Command(buf[1]) > socks5.CommandMax {
		return "", errors.Errorf("invalid command %d", buf[1])
	}

	addrType := socks5.AddressType(buf[3])
	...
	default:
		return "", errors.Errorf("invalid or unsupported address type(%d)", buf[3])
	}
